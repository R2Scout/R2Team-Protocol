...
Preserve completed/remaining/blockers, evidence and next action in the same TASK/artifacts at the publication boundaries. Return PASS/FAIL/BLOCKED only to `result_to_executor_id`/`result_to_route` using `git_checkpoint_then_tracker`: increment `handoff_seq`, make that recipient the next owner, copy its route to `owner_route`, set the following stage's result fields, publish exact candidate/evidence and next owner/status/role to the accepted default branch, then post its pointer in the new owner route. Do not substitute the issuer, publisher, local PM or chat parent; a remote executor never uses direct chat for this return. If publication/delivery is unavailable, report ROUTE_REQUIRED/LOCAL_ONLY/SYNC_REQUIRED/NOT_DELIVERED accurately. Bounded helper replies may return directly; no mandatory ACK, MSG or separate report for TASK 2.4.
