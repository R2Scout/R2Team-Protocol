...
For every baton transition, publish the TASK checkpoint to the accepted default branch first. Then update exactly the new `owner_route` with the target executor, TASK path, accepted commit, work ref/candidate and expected action. The current `result_to_executor_id`/`result_to_route` determines that new owner; the checkpoint then records the following stage's return route. On return, the recipient uses its declared result fields and an authorized transition rather than addressing whichever account last commented, its local PM, Task Issuer or chat parent. For a remote executor this provider pointer is mandatory; a local direct exchange may only announce it after publication.
