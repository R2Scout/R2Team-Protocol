...
- Before executing, show a brief intake Route Card: TASK/revision and current owner/`owner_route`, Task PM route, exact result recipient/route, `git_checkpoint_then_tracker` delivery policy, executor mode/location, boundaries, needed human action and Starting/BLOCKED. The chat sender/physical parent is non-authoritative unless it exactly matches the declared result fields. No separate ACK.
...
- For every ownership transfer, use the same TASK as the durable baton: follow its authorized transition, increment `handoff_seq`, set exact next owner/`owner_route`/role/status and the **following stage's** `result_to_executor_id`/`result_to_route`, publish to the accepted default branch, then notify the new `owner_route`. Independent TASK delivery is always `git_checkpoint_then_tracker`; remote roles discover this assignment by executor ID and never return it to a local PM or chat.
...
- Queue checks and work-session entry use current remote default-branch metadata for all assignments to the configured executor, including unknown TASK IDs. Report `executor_queue` versus explicitly requested `task_only`, checked SHA and incomplete coverage. A known TASK on HOLD does not mean the executor has no work; an unchanged cursor does not acknowledge a pending READY task. Failed/incompatible checks are `QUEUE_CHECK_INCOMPLETE`.
- Every assignment carries explicit Task Issuer and Project PM routes. Use ASK, LOOP or ESCALATE_PM under [the question contract](OPERATING_COMMUNICATION.md#question-routes); inspect addressed requests even when another executor owns the TASK. Answers return to the requester, and only dependent work waits. PM arbitration cannot replace evidence, QA or required human authority.
- A remote role starts/resumes by invoking its bounded internal COO or same-chat update check for its own executor IDs. `RETURNED_TO_PARENT` means only the internal COO result to its invoking role; it is not a product-task handoff or a route to PM. A discovered TASK returns to that role, which then uses `r2team-work start`; no self-wake or local registry is involved.
