...
Operational `update`/`check`, session entry and before-idle passes default to `executor_queue` scope. An explicit request limited to one TASK uses `task_only`: report that ID and that the rest of the queue was not examined. A configured watch list is a cache, not an exhaustive queue or permission to narrow an executor-wide request. A scheduled prompt pinned to one old TASK cannot certify executor-wide inactivity; report the mismatch without changing the schedule.
...
2. Store `last_seen_default_branch_sha` only in approved ignored `.codex-local` state. On later passes resolve/fetch the current remote default branch, compare from that SHA, and inspect only added/changed TASK frontmatter plus relevant TEAM changes. Keep unresolved assignments/questions from the last complete baseline in the ignored cache or current pass; unchanged SHA does not make them handled. Use `handoff_seq` to distinguish a new baton from a previously handled stage. A cursor without the pending set requires a new bounded baseline. A failed or non-fast-forward comparison does not advance the cursor; reconcile or repeat the bounded baseline.
...
Also match non-closed TASK `questions` metadata addressed to watched executors, independent of `owner_executor_id`. Follow the recorded Task Issuer/Project PM provider routes for newer questions and answers; use exact linked items or supported scoped provider queries, never global account history. An unanswered provider event may precede its Git checkpoint: surface it to the authorized publisher and do not treat it as a scope change. Read only matched bodies. `OPEN`/`ESCALATED` needs the respondent; `ANSWERED` needs the original requester to validate/close it. Return these to the parent just like assignments; COO does not answer on another role's behalf.

Before reporting no available work, require a complete current executor queue. A task-local HOLD/BLOCKED does not block another eligible READY TASK. Preserve separately declared executor/capacity restrictions. Unknown mappings, missing source/route or failed reads yield `QUEUE_CHECK_INCOMPLETE`; never convert them into no assignments. Discovery/notification receipts do not prove intake, and unaccepted READY work remains pending across unchanged passes. Automatic passes may stay quiet on unchanged state, but manual checks must not say no work merely because no new delta arrived.

Report one compact result: `scope: executor_queue|task_only`, watched executor IDs, verified default-branch SHA, actionable/pending/held TASK IDs, addressed questions and next action; include unexamined scope or incomplete reads. These are response fields, not a new log. A check reports/returns findings; only an authorized work-session request permits the parent to execute through r2team-work.
...
For every match report `previous_owner_executor_id`, `owner_executor_id`, `owner_route`, `handoff_seq`, `result_to_executor_id`, `result_to_route`, `delivery_policy`, status and next action. The role returns its stage result only by making that declared result executor/route the next owner/owner route in the next authorized TASK checkpoint; it does not choose a recipient from chat history, task issuer, publisher, local PM or internal COO parent.

## Wake for check only

An internal COO subagent returns the delta to parent as `RETURNED_TO_PARENT` and does not send external notifications. This means only the helper's advisory return to its invoking role; it is never a TASK handoff or product-result route. A same-chat check returns `ACTION_FOUND_LOCAL`. Neither mode performs delivery or requires `.codex-local/THREAD_REGISTRY.md`. A standalone COO needs current notify_local authorization, exact recipient mapping and designation as the single dispatcher for the watched recipients.
