...
At stage completion, read the TASK's authorized transition table and `result_to_executor_id`. Publish the exact work/evidence checkpoint, increment `handoff_seq`, set the authorized next owner/status/role/result recipient and get that TASK checkpoint onto the accepted default branch through the normal protected workflow. Then update the linked Issue/PR with the exact accepted commit and target executor. If the transition is missing or publication fails, stop at `ROUTE_REQUIRED`, `LOCAL_ONLY` or `SYNC_REQUIRED`; do not claim handoff or select a recipient from chat history.

## Inspect or start

Bare/unknown commands show help; task without an action never executes. For `$r2team-work start` without a TASK ID, first perform the same bounded internal/same-chat assignment discovery used by r2team-coo for the current executor: accepted default-branch TASK frontmatter, exact `owner_executor_id`, actionable status and `handoff_seq`. If exactly one current assignment is found, select it and continue to intake; if several exist, ask for priority. If only a prepared `PENDING_REGISTRATION` or `PENDING_CAPACITY` TASK exists, report its exact gate and do not ask for a redundant approval. If none exists, ask PM through the recorded route. Never require a local registry or self-wake for this path.
