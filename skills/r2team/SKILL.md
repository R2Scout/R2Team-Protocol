...
Use Setup team as PM/authorized delegate. Clarify participant/provider account, functions, executor IDs, rights and one explicit mode per executor: existing local standalone task, new local standalone task, persistent/ephemeral subagent, or remote/manual participant. PM is the sole mandatory coordinating executor; any other function can be combined. A persistent subagent with its own queue may be a named executor with a parent; an ephemeral helper is not registered.
...
Compare accepted project rules with actually loaded work/COO skills and saved entry/watch prompts. Confirm executor-wide unknown-TASK discovery, task-only coverage reporting, and explicit Task Issuer/Project PM routes. Every role can ASK the issuer, LOOP on disagreement or ESCALATE_PM directly within the adopted question contract. Verify how addressed questions are found even when another executor owns the TASK. A newer skill file alone is not adoption evidence.
...
Explicitly confirm/update Task Issuer and Project PM response routes and carry unresolved questions, current recipients, statuses and decisions forward. A change of publisher does not implicitly change issuer. Keep both routes even when they resolve to the same executor.
