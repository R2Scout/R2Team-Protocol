...
`R2TEAM_MASTER.md` may bootstrap or route an administration request to this skill. Preserve its local return route when provided, but obtain project decisions from PM. Master is a consultant/controller, not a TASK owner or substitute PM.

The distribution repository is https://github.com/R2Scout/R2Team-Protocol; templates live at its root under templates/. Prefer the PM-provided full commit SHA. A candidate branch is usable only through its explicitly approved full commit and is not a release tag; never silently substitute main/latest. Obtain the package only into a confirmed safe destination with authorized access. For a new project, missing TEAM/TASK is expected: use the trusted package START/Setup to create them after confirmation, not an existing-team registration prerequisite. User-installed skills are not the complete template bundle.

### add — PM prepares an invitation

Use Setup team as PM/authorized delegate. Clarify participant/provider account, functions, executor IDs, rights, `location: local | remote`, direct-exchange policy and one explicit mode per executor: existing local standalone task, new local standalone task, persistent/ephemeral subagent, or remote/manual participant. Every independent task return is `git_checkpoint_then_tracker`; remote/manual executors do not receive/need chat routes. PM is the sole mandatory coordinating executor; any other function can be combined. A persistent subagent with its own queue may be a named executor with a parent; an ephemeral helper is not registered.
Configure one COO mode for the participant: `internal` by default, `same_chat`, or optional `standalone`. Offer a separate COO chat exactly once and persist accepted/declined/not_supported. If accepted, ask whether to connect an existing chat or create a new one; creation requires the user's explicit request. Heartbeat is a separate opt-in and stays off.
...
Before inviting a remote participant, publish an onboarding TASK and a prepared first product TASK. The onboarding TASK contains `registration_id`, an exact Issue/Work Item URL, `onboarding_result_to_executor_id`, objective `auto_accept_if` evidence, `first_task_id`, and `IMMEDIATE_RESERVED` or `QUEUED_AFTER_REGISTRATION`. Empty `auto_accept_if` requires a new PM decision. Do not use an unregistered commit comment as the primary inbox. Return a portable invitation containing those fields plus repo URL, participant/executor IDs, accepted TEAM path/ref, protocol path/ref and prepared TASK/branch. No secrets or another machine's task IDs/paths. For a local standalone executor also create a separate ephemeral launch prompt with PM task ID, exact register/connect commands and mandatory direct return. Store routes only in ignored `.codex-local/THREAD_REGISTRY.md`. Invite after publication; otherwise mark LOCAL_ONLY. Unconfirmed onboarding is `NOT_DELIVERED`.

### register — participant joins an existing team

Follow Setup join. Verify repo/protocol source and actual provider identity against the current accepted TEAM, not solely the name in the prompt or an outdated invitation SHA. A matching invited/pending participant plus onboarding TASK is the expected pre-activation state. An absent/changed entry, or an inactive entry without that matching registration, is a blocker to PM—not permission to add yourself or create another PM.
...
One participant can have multiple executors/functions. If this chat's executor is unambiguous, run connect in this pass; otherwise ask which assigned executor to adopt. Repeated register reuses existing identity and checked facts, not duplicate TEAM entries. Post a structured `REGISTRATION_RESULT` with `READY_FOR_ACTIVATION`, evidence and questions to the exact onboarding channel, addressed to `onboarding_result_to_executor_id`. Use `ONBOARDING_QUESTION` for blockers. Do not begin product work during registration.
...
Compare accepted project rules with actually loaded work/COO skills and saved entry/watch prompts. Confirm executor-wide unknown-TASK discovery, task-only coverage reporting, and explicit Owner/Task Issuer/Project PM/**Result Recipient** routes. Confirm that `RETURNED_TO_PARENT` is helper-only and that a remote executor returns by making `result_to_*` the next Owner/`owner_route`, not its local PM/chat. Every role can ASK the issuer, LOOP on disagreement or ESCALATE_PM directly within the adopted question contract. Verify how addressed questions are found even when another executor owns the TASK. A newer skill file alone is not adoption evidence.
Do not take over another live task's assignment without an approved handoff. Optional task mapping is local and ignored. A named persistent subagent cross-checks through its parent; an ephemeral helper does not register as a TEAM executor.
Report readiness/questions in the exact onboarding Issue/Work Item if permitted; publisher records material state in its organisational TASK. If no channel/access, ask the participant to relay and mark NOT_DELIVERED. After PM publishes accepted registration, run the configured COO pass: start a prepared TASK only when its accepted status is `READY` for this executor. `PENDING_CAPACITY` is an explicit queue state, not a request for another hidden approval. No extra ACK/MSG/report.
...
Explicitly confirm/update Owner, Task Issuer, Project PM and Result Recipient routes and carry unresolved questions, current recipients, statuses and decisions forward. A change of publisher does not implicitly change issuer, owner or result recipient. Keep all routes even when they resolve to the same executor.
