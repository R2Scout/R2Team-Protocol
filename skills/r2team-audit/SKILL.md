...
For each active participant verify exactly one COO mode (`internal`, `same_chat`, or `standalone`), exact watched executor IDs, persisted one-time separate-chat offer result, and heartbeat false unless explicitly authorized. For remote onboarding verify `registration_id`, exact provider channel, PM result recipient, structured result/question event, prepared first TASK and explicit activation/capacity mode. Missing route or disabled COO capability is a recoverability gap, not evidence of inactivity.
...
- Explicit Owner/Task Issuer/Project PM/Result Recipient executor/routes separate from publisher; every independent TASK has `owner_route`, `result_to_route` and `delivery_policy: git_checkpoint_then_tracker`. Verify transitions make the current result recipient the next owner, remote executors do not depend on local-chat routes, and `RETURNED_TO_PARENT` remains helper-only. Questions carry mode/status/Decision and current recipient. Verify addressed-question discovery on others' TASKs, answer return to requester, preserved unresolved questions on transfer, and PM arbitration within evidence/QA/human gates.
